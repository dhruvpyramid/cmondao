...
export default function Home() {
  ...
  const handleGetMON = async () => {
    try {
      ...
      const hash = await wc.writeContract({ address: CONTRACTS.MONToken.address, abi: CONTRACTS.MONToken.abi, functionName: 'faucet' });
      ...
      <header className="border-b border-[#30363d] bg-[#0d1117]/95 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            {!authenticated ? (
              <button onClick={login} className="bg-[#238636] hover:bg-[#2ea043] text-white font-semibold px-5 py-2.5 rounded-lg transition">Connect wallet</button>
            ) : (
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-[#161b22] border border-[#30363d] rounded-lg">
                  ...
                  <span className="text-sm font-mono">{user?.wallet?.address?.slice(0, 6)}...{user?.wallet?.address?.slice(-4)}</span>
                </div>
                <button onClick={logout} className="px-4 py-2 bg-[#21262d] hover:bg-[#30363d] rounded-lg text-sm transition">Disconnect</button>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="card p-6">
                ...
                <div className="bg-[#0d1117] border border-[#30363d] rounded-lg p-4 mb-6">
                  ...
                  <button onClick={handleGetMON} disabled={loading} className="w-full py-2 bg-[#21262d] hover:bg-[#30363d] rounded-lg text-sm font-medium transition disabled:opacity-50">
                    {loading ? 'Processing...' : 'Get test MON'}
                  </button>
                </div>
                ...
                <button onClick={handleStake} disabled={loading || !stakeAmount} className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed">
